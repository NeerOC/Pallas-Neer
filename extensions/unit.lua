...
function WoWUnit:InCombatWithMe()
  for k, v in pairs(self.ThreatTable) do
    if Me.Guid == v.Guid or Me.Pet and Me.Pet.Guid == v.Guid then return true end
  end
...
function WoWUnit:IsHealImmune()
  for _, immune in pairs(immunes.heal) do
    if self:HasAura(immune) then
      return true
    end
  end
...
function WoWUnit:IsValidTarget()
  ...
  local inDistance = self:GetDistance(Me.ToUnit) <= 50
...
function WoWUnit:AngleTo(target)
  return self:AngleToPos(self.Position, target.Position)
end

function WoWUnit:IsFacing(target, ang)
  if target == Me and self == Me then return true end

  local angle = self:AngleTo(target)
  ang = ang or 90
...
---@return integer
---@param target WoWUnit The target of the unit, pet, me, another player?
function WoWUnit:GetThreatValue(target)
  local threat = self.ThreatTable
  for _, v in pairs(threat) do
    if v.Guid == target.Guid then
      return v.RawValue
...
function WoWUnit:GetUnitsAround(dist)
  ...
  for _, u in pairs(units) do
    if self:GetDistance(u) < dist and not u.Dead and Me:CanAttack(u) then
...
function WoWUnit:IsBehind(target)
  return self:IsFacing(target) and not target:IsFacing(self)
end

function WoWUnit:IsCastingFixed()
  return self.IsCastingOrChanneling and self.CurrentSpell
end

function WoWUnit:IsPartyMember()
  local group = WoWGroup(GroupType.Auto)

  return group:GetMemberByGuid(self.Guid) ~= nil
end

function WoWUnit:InCombatWithPartyMember()
  local group = WoWGroup(GroupType.Auto)
  local target = self.Target

  if target and group:GetMemberByGuid(target.Guid) ~= nil then return true end
